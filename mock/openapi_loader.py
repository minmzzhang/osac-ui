...
def _status_has_message(spec: dict, schema_ref: str) -> bool:
    """Return True when the resource status schema defines a top-level message field."""
    schema = spec.get("components", {}).get("schemas", {}).get(schema_ref, {})
    status_prop = schema.get("properties", {}).get("status", {})
    ...
    return "message" in status_schema.get("properties", {})


def status_message_resource_types(spec: dict) -> frozenset[str]:
    """Resource types whose status schema includes a top-level message field."""
    return frozenset(
        rc.name
        for rc in discover_resources(spec)
        if rc.status_has_message
    )


@dataclass(frozen=True)
class ResourceSchemaAllowlist:
    top_level: frozenset[str]
    spec: frozenset[str] | None
    status: frozenset[str] | None


def _nested_property_names(spec: dict, schema: dict, field: str) -> frozenset[str] | None:
    ...
    if ref:
        ...
        if not resolved:
            return frozenset()
        return frozenset(resolved.get("properties", {}).keys())

    if field_def.get("type") == "object" and "properties" in field_def:
        return frozenset(field_def["properties"].keys())

    return frozenset()


def build_resource_allowlists(spec: dict) -> dict[str, ResourceSchemaAllowlist]:
    """Build OpenAPI property allowlists for scenario resource sanitization."""
    allowlists: dict[str, ResourceSchemaAllowlist] = {}
    schemas = spec.get("components", {}).get("schemas", {})

    for rc in discover_resources(spec):
        if not rc.schema_ref:
            continue
        schema = schemas.get(rc.schema_ref, {})
        allowlists[rc.name] = ResourceSchemaAllowlist(
            top_level=frozenset(schema.get("properties", {}).keys()),
            spec=_nested_property_names(spec, schema, "spec"),
            status=_nested_property_names(spec, schema, "status"),
        )

    return allowlists


def _filter_mapping(data: dict, allowed: frozenset[str]) -> None:
    for key in list(data.keys()):
        if key not in allowed:
            del data[key]


def sanitize_scenario_resource(
    allowlists: dict[str, ResourceSchemaAllowlist],
    resource_type: str,
    obj: dict,
) -> None:
    """Drop scenario fields that are not defined in the OpenAPI resource schema."""
    allowlist = allowlists.get(resource_type)
    if allowlist is None:
        return

    _filter_mapping(obj, allowlist.top_level)

    if allowlist.spec is None:
        obj.pop("spec", None)
    elif isinstance(obj.get("spec"), dict):
        _filter_mapping(obj["spec"], allowlist.spec)

    if allowlist.status is None:
        obj.pop("status", None)
    elif isinstance(obj.get("status"), dict):
        _filter_mapping(obj["status"], allowlist.status)
...
def discover_resources(spec: dict) -> list[ResourceConfig]:
    ...
    for rc in resources.values():
        if rc.schema_ref:
            ...
            rc.status_has_message = _status_has_message(spec, rc.schema_ref)
