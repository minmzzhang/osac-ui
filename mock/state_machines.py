...
async def run_ticker(
    store: ResourceStore,
    interval: float = 0.5,
    status_message_types: frozenset[str] | None = None,
) -> None:
    """Background task that advances resource state machines."""
    status_message_types = status_message_types or frozenset()
    while True:
...
async def _check_fail_after(
    ...
    now: datetime,
    status_message_types: frozenset[str],
) -> None:
